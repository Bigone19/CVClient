...
CResTransRecevicer* CResTransRecevicer::m_recevicer = nullptr;

CResTransRecevicer::CResTransRecevicer(QObject* parent /*= nullptr*/)
	: QObject(parent)
	, m_isCancel(false)
	, m_isRunning(false)
	...
	, m_recvSizeBefore(0)
{
	QString resultDir = QDir::currentPath() + "/results/";
	...
	m_tcpServerComm->listen(QHostAddress::Any, PORT_RESCOMM);
	m_tcpServerFile->listen(QHostAddress::Any, PORT_RESFILE);

	m_timerProgress = new QTimer(this);
	connect(m_timerProgress, &QTimer::timeout, this, &CResTransRecevicer::progress);
	m_timeOutProgress = new QTimer(this);
	connect(m_timeOutProgress, &QTimer::timeout, this, &CResTransRecevicer::timeOutCheck);
	...
	qDebug() << QString::fromLocal8Bit("<=========处理结果接收端初始化完成============>");

}

CResTransRecevicer::~CResTransRecevicer()
{
	m_cacheList.clear();
}

void CResTransRecevicer::setResultDir(const QString& dirPath)
{
	if (dirPath.isEmpty())
	{
		qDebug() << ERROR_CODE_1;
...
void CResTransRecevicer::beginSave()
{
	while (true)
	{
		if (m_isCancel)
		...
		else
		{
			...
			if (m_recvSize == m_fileSize)
			{
				...
				QDir::setCurrent(m_resultDir);
				m_file.close();
				qDebug() << QString::fromLocal8Bit("文件缓存路径:") << m_resultDir;
				break;
			}
			QThread::msleep(10);
		}
...
void CResTransRecevicer::quitFileTransferThread()
{
	while (m_isRunning)
	{
		m_isCancel = true;
...
void CResTransRecevicer::send_command(SEND_TYPE code, int ret, QString additional /*= ""*/)
{
	QByteArray data;
...
void CResTransRecevicer::progress()
{
...
void CResTransRecevicer::timeOutCheck()
{
...
void CResTransRecevicer::on_connect_c()
{
...
void CResTransRecevicer::on_connect_f()
{
...
void CResTransRecevicer::on_read_command()
{
...
void CResTransRecevicer::on_read_file()
{
	if (m_startTimeFlag)
	{
...
void CResTransRecevicer::onFileError(QAbstractSocket::SocketError)
{
	qDebug() << "file_socket error: " << m_fileSocket->errorString();
}

void CResTransRecevicer::onCommandError(QAbstractSocket::SocketError)
{
